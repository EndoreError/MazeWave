...
class MazeGenerator:
    ...
    def generate_dfs(self) -> tuple[Maze, tuple[int, int], tuple[int, int]]:
        """
        ...
        maze_barrier = Maze(self.width * 2 + 1, self.height * 2 + 1)

        stack = [self.start]
        visited = {self.start}
        maze.set_terrain(self.start, ROAD)
        maze_barrier.set_terrain((self.start[0] * 2 + 1, self.start[1] * 2 + 1), ROAD)
        ...
        while stack:
            ...
            if neighbours:
                ...
                maze.set_terrain(now_neighbour, ROAD)
                maze_barrier.set_terrain((now_neighbour[0] * 2 + 1, now_neighbour[1] * 2 + 1), ROAD)
                wall_point = (now_neighbour[0] + stack[-1][0] + 1, now_neighbour[1] + stack[-1][1] + 1)
                maze_barrier.set_terrain(wall_point, ROAD)
        ...
        return maze_barrier, physical_start, physical_finish